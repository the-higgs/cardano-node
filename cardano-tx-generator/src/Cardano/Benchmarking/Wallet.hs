...
benchmarkTransaction :: forall era. IsShelleyBasedEra era
  ...
  -> Target
  -> Either String (Wallet, Tx era)
benchmarkTransaction wallet numInputs numOutputs targetNode = do
...
benchmarkWalletScript :: forall era .
  ...
  -> Target
  -> WalletScript era
benchmarkWalletScript wRef maxCount numInputs numOutputs targetNode
  = WalletScript (modifyMVarMasked wRef genTx)
  where
    nextCall = benchmarkWalletScript wRef maxCount numInputs numOutputs targetNode
    genTx :: Wallet -> IO (Wallet, WalletStep era)
    genTx w = if walletSeqNumber w > maxCount
      then return (w, Done)
      else case benchmarkTransaction w numInputs numOutputs targetNode of
