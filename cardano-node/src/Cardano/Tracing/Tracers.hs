      ...
      case pointSlot p of
        ...
        At slotNo -> do
          ...
          when fresh $ do
            ...
            sendEKGDirectDouble
              ekgDirect
              "cardano.node.metrics.blockfetchclient.blockdelay.cdf1s"
              cdf1s
            sendEKGDirectDouble
              ekgDirect
              "cardano.node.metrics.blockfetchclient.blockdelay.cdf2s"
              cdf2s
            sendEKGDirectDouble
              ekgDirect
              "cardano.node.metrics.blockfetchclient.blockdelay.cdf5s"
              cdf5s
            when (delay > 5) $
  ...
  where
    ...
    ipgcTracer = Tracer $ \msg -> case msg of
      (TrInboundGovernorCounters InboundGovernorCounters {
          establishedPeersRemote,
          warmPeersRemote,
          hotPeersRemote
        }) -> do
          sendEKGDirectInt ekgDirect "cardano.node.metrics.inbound-governor.established"
                                      establishedPeersRemote
